...
export default class EXS_Examination_ListScreen extends React.Component {

  static navigationOptions = ({ navigation }) => ({
    title: 'รายการตรวจข้อสอบ',
    headerStyle: {
  ...
  ShowEXS_Exam = async () => {
    await fetch(
      'http://172.17.146.223/traineedrive_F/public/api/exs/trace'
      //'http://172.17.146.223/traineedrive/public/api/nfrl2/notebook'
      ...
      .then((responseJson) => {
        ...
        this.setState(
          {
            isLoading: false,
            dataSource: responseJson.trace,
          },
          function () { }
        );
        this.arrayholder = responseJson.trace;
      })
      .catch((error) => {
        console.error(error);
      });
      ...
      <View style={styles.container}>
        ...
        >
          <View style={styles.searchSection}>
            <Icon style={styles.searchIcon} id="search" size={20} color='#696969' />
            <TextInput
              style={styles.input}
              onChangeText={(text) => this.SearchFilterFunction(text)}
              value={this.state.text}
              underlineColorAndroid='black'
              placeholder="ค้นหาจากลำดับรายการ"
            />
          </View>
            ...
            renderItem={({ item }) =>
              ...
              >
                ...
                <View style={styles.buttonSection}>
                  <Button
                    onPress={() => {navigate('EXS_Examination_View', {
                       param_id: item.id, //เปิด id
                       param_nb_code: item.nb_code,
                       param_nb_brand: item.nb_brand,
                       param_nb_details: item.nb_details,
