...
export default class EXS_Reserve_StatusScreen extends React.Component {
  ...
  ShowEXS_Reserve = async () => {
    await fetch(
      'http://172.17.146.223/traineedrive_F/public/api/exs/reserve/'
          ...
          <FlatList
            ...
            renderItem={({ item }) =>
              ...
              >
                ...
                <Text style={styles.text}>วันที่จองตรวจข้อสอบ : {item.examination_date}</Text>
                <Text style={styles.text}>สถานะการจอง : {item.reserve_status}</Text>
                <Text style={styles.text}>หมายเหตุ : {item.remark}</Text>
